...
namespace zSpace
{
	...
	ZSPACE_INLINE void zTsSDFBridge::createSplitMesh(double width)
	{
		...
		for (zItMeshEdge e(*o_guideMesh); !e.end(); e++)
		{
			if (guide_MedialEdgesBoolean[e.getId()])
			{
				...
				else
				{
					...
					else faceColors.push_back(zColor(1, 0, 0, 1));
		...
		for (zItMeshEdge e(*o_guideMesh); !e.end(); e++)
		{
			...
			if (!cHe.getNext().getNext().getEdge().onBoundary())
			{
				...
				zColor currentFaceColor = faceColors[guideHalfEdge_planeFace[cHe.getNext().getId()][0]] /*cHe.getFace().getColor()*/;

				zColor tempFaceColor = faceColors[guideHalfEdge_planeFace[temp.getPrev().getId()][0]] /*temp.getFace().getColor()*/;
				zColor red(1, 0, 0, 1);

				if (currentFaceColor == tempFaceColor && currentFaceColor == red)
				{
					//if (guideHalfEdge_planeFace[cHe.getNext().getId()].size() == 0) continue;
	...
	{
		if (blockId >= macroblocks.size() ) return zPointArray();

		return macroblocks[blockId].intersectionPoints;
	...
	ZSPACE_INLINE vector<zTransform> zTsSDFBridge::getBlockFrames(int blockId)
	{
		...
		if (blockId >= macroblocks.size()) return vector<zTransform>();

		return macroblocks[blockId].sectionFrames;
	}
	...
	ZSPACE_INLINE zObjGraphPointerArray zTsSDFBridge::getBlockGraphs(int blockId, int& numGraphs)
	{
		...
		if (blockId >= macroblocks.size()) return out;

		numGraphs = macroblocks[blockId].o_sectionGraphs.size();

		if(numGraphs == 0 )return out;
		
		for (auto& graph : macroblocks[blockId].o_sectionGraphs)
		{
			out.push_back(&graph);
		}
	...
	ZSPACE_INLINE zObjGraphPointerArray zTsSDFBridge::getBlockContourGraphs(int blockId, int& numGraphs)
	{
		...
		numGraphs = macroblocks[blockId].o_contourGraphs.size();

		if (numGraphs == 0)return out;

		for (auto& graph : macroblocks[blockId].o_contourGraphs)
		{			
			out.push_back(&graph);
		}
	...
	ZSPACE_INLINE void zTsSDFBridge::computeBlocks(float printLayerDepth)
	{
		...
		for (auto fV : printMedialVertices)
		{
			...
			do
			{
				...
				{
					macroblocks.push_back(zMacroBlock());
					macroblocks[macroBlockCOunter].id = macroBlockCOunter;
				...
				do
				{
					if (printMedialVerticesBoolean[temp.getVertex().getId()])
					{
						...
						if (!edgeVisited[temp.getEdge().getId()])
						{
							...
							//left block
							int l_id = macroblocks[macroBlockCOunter ].leftBlocks.size();
							macroblocks[macroBlockCOunter].leftBlocks.push_back(zBlock());

							zItMeshEdge l_e = temp.getNext().getNext().getEdge();
							macroblocks[macroBlockCOunter].leftBlocks[l_id].id = l_e.getId();
							computeBlockFaces_fromEdge(macroblocks[macroBlockCOunter].leftBlocks[l_id], l_e);

							//right block
							int r_id = macroblocks[macroBlockCOunter].rightBlocks.size();
							macroblocks[macroBlockCOunter].rightBlocks.push_back(zBlock());

							zItMeshEdge r_e = temp.getSym().getNext().getNext().getEdge();
							macroblocks[macroBlockCOunter].rightBlocks[r_id].id = r_e.getId();
							computeBlockFaces_fromEdge(macroblocks[macroBlockCOunter].rightBlocks[r_id], r_e);
						}
				...
				if (incrCounter)
				{
					printf("\n edgeCOunter %i: l %i , r %i ", edgeCOunter, macroblocks[macroBlockCOunter].leftBlocks.size(), macroblocks[macroBlockCOunter ].rightBlocks.size());
		...
		printf("\n macroBlockCOunter : %i ", macroBlockCOunter);
	...
	ZSPACE_INLINE bool zTsSDFBridge::planarisePlaneMesh(zDomainFloat& deviation,float dT, zIntergrationType type, float tolerance, int numIterations)
	{
		...
		for (int k = 0; k < numIterations; k++)
		{
			...
			for (auto fPair : planeFace_targetPair)
			{
				...
				else if(planeFace_colors[fCounter] == yellow)
				{
					/*if (abs(fNormalTargets[fCounter].z < 0.1))
					{
						fNormalTargets[fCounter].z = fNormals[fCounter].z * coreUtils.zSign(fNormals[fCounter].z);
						fNormalTargets[fCounter].normalize();
					}*/
				}
				fCounter++;
