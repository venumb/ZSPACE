...
namespace zSpace
{
				...
				do
				{
					if (printMedialVerticesBoolean[temp.getVertex().getId()])
					{
						...
						if (!edgeVisited[temp.getEdge().getId()])
						{
							...
							addConvexBlocks(macroblocks[macroBlockCOunter], temp);							
						}
	...
	{
		if (blockId >= macroblocks.size()) return;

		zFnMesh  fn_guideSmoothMesh(o_guideSmoothMesh);

		zObjMeshArray splitMeshes;
		splitMeshes.assign(macroblocks[blockId].rightBlocks.size() + macroblocks[blockId].leftBlocks.size(), zObjMesh());
		...
		//{
			...
			for (auto fId : macroblocks[blockId].rightBlocks[1].faces)
			{
		...
		for (auto& b : macroblocks[blockId].leftBlocks)
		{
			zPointArray splitPlanes_origins;
			zVectorArray splitPlanes_normals;
	...
	ZSPACE_INLINE void zTsSDFBridge::addConvexBlocks(zMacroBlock& mBlock, zItMeshHalfEdge& guideMesh_halfedge)
	{
		//left block
		if (!guideMesh_halfedge.getNext().getNext().getEdge().onBoundary())
		{
			int l_id = mBlock.leftBlocks.size();
			mBlock.leftBlocks.push_back(zBlock());
			...
			if (!guideMesh_halfedge.getSym().getNext().getNext().getEdge().onBoundary())
			{
				...
				computeBlockFaces_fromEdge(mBlock.leftBlocks[l_id], l_e);
			}
			else
			{
		...
		if (!guideMesh_halfedge.getSym().getNext().getNext().getEdge().onBoundary())
		{
			...
			mBlock.rightBlocks.push_back(zBlock());
			...
			if (!guideMesh_halfedge.getNext().getNext().getEdge().onBoundary())
			{
				...
				computeBlockFaces_fromEdge(mBlock.rightBlocks[r_id], r_e);
			}
			else
			{
