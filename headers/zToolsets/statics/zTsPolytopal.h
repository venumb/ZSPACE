...
{	   
	...
	*/
	...
	public:
		...
		vector<int> primal_internalEdgeIndex; // -1 for GFP or SSP edges 
		vector<int> internalEdgeIndex_primalEdge;

		vector<int> primal_internalVertexIndex; // -1 for GFP or SSP vertex 
		vector<int> internalVertexIndex_primalVertex;

		vector<zVector> primalVertexPositions;
		vector<zVector> primalFaceCenters;
		vector<zVector> primalFaceNormals;
		vector<double> primalFaceAreas;

		vector<vector<int>> primalFace_Volumes;

		vector<bool> GFP_SSP_Face;
		vector<bool> GFP_SSP_Edge;
		vector<bool> GFP_SSP_Vertex;

		vector<vector<int>> primalVertex_ConnectedPrimalFaces;
		vector<int> Y_vertex;
		
		vector<zVector> userSection_points;
		...
		*	\since version 0.0.2
		*/
		void createFormFromForce( zColor edgeCol = zColor(0.75, 0.5, 0, 1));
		...
		*	\since version 0.0.2
		*/
		bool equilibrium(bool computeTargets, double minmax_Edge, double dT, zIntergrationType type, int numIterations = 1000, double angleTolerance = EPS, bool colorEdges = false, bool printInfo = false);
	...
	protected:
		...
		*	\since version 0.0.3
		*/		
		void get_BMatrix(mat &A, double threshold, int &rank, mat &B, vector<bool> &indEdges);
		...
		*	\since version 0.0.3
		*/
		void get_YVertex(vector<int> &Y_vertex);
		...
		*	\since version 0.0.3
		*/
		void getUserSection(int face_numEdges, vector<zVector> &sectionPoints);
