...
namespace zSpace
{
	...
	vector<double> internal_fAreas;

	void loadPath_func(const alglib::real_1d_array &Q, double &func, void *ptr)
	{
		func = 0;
...
namespace zSpace
{
	...
	ZSPACE_INLINE zTsPolytopal::zTsPolytopal(zObjGraph &_formObj, vector<zObjMesh> &_forceObjs, vector<zObjMesh>  &_polytopalObjs)
	{
		formObj = &_formObj;
	...
	ZSPACE_INLINE void zTsPolytopal::createForceFromFiles(vector<string> filePaths, zFileTpye type)
	{
	...
	ZSPACE_INLINE void zTsPolytopal::createFormFromForce(zColor edgeCol)
	{
	...
	ZSPACE_INLINE bool zTsPolytopal::equilibrium(bool computeTargets, double minmax_Edge, double dT, zIntergrationType type, int numIterations, double angleTolerance, bool colorEdges, bool printInfo)
	{
			...
			for (int j = 0; j < fnForces.size(); j++)
			{
				...
				for (zItMeshVertex v(*forceObjs[j]); !v.end(); v++)
				{
					...
					if (!chkExists)
					{
						...
						primalVertex_ConnectedPrimalFaces.push_back(vector<int>());
	...
	ZSPACE_INLINE void zTsPolytopal::getDual(double threshold)
	{
		VectorXd q;
		...
		positions.assign(C_fc.cols(), zVector(3, 0, 0));
	...
	ZSPACE_INLINE void zTsPolytopal::getPolytopal(int forceIndex, double offset, double param, int subdivs)
	{
		...
		for (zItMeshEdge e(*forceObjs[forceIndex]); !e.end(); e++)
		{
			...
			vector<zVector> tempPosns;
			zVector pos0 = e.getHalfEdge(1).getVertex().getPosition();
			zVector pos1 = e.getHalfEdge(0).getVertex().getPosition();
			...
			for (int i = 0; i < eFaces.size(); i++)
			{
				...
				if (gotFace != volumeFace_PrimalFace.end())
				{
					...
					if (!GFP_SSP_Face[globalFaceId])
					{
						...
						{
							zVector vCenter_graphPos = formPositions[forceIndex];
							...
							for (int j = 0; j < tempPosns.size(); j++)
							{
								...
								zVector newPos = volCenter + (dir_volCenter_0 * len0 *offset);
						...
						zVector fCenter = fCenters[eFaces[i]];
						...
						for (int j = 0; j < tempPosns.size(); j++)
						{
							...
							zVector newPos = fCenter + (dir_fCenter_0 * len0 *offset);
	...
	ZSPACE_INLINE void zTsPolytopal::getPolytopal_Profile(int forceIndex, int user_nEdges, double user_edgeLength, double offset, double param, int subdivs)
	{
		...
		vector<int> v_f_profileIndex;
		v_f_profileIndex.assign(n_v * n_f, -1);

		vector<int> temp_CornerIds;
		vector<zVector> sectionPoints;
		...
		for (zItMeshFace f(*forceObjs[forceIndex]); !f.end(); f++)
		{
			...
			if (gotFace != volumeFace_PrimalFace.end())
			{
				...
				if (userSection_numEdge == 4 || userSection_numEdge == 5 || userSection_numEdge == 6) temp_CornerIds = vector<int>{ 0, 2, 4 };

				else if (userSection_numEdge == 8)temp_CornerIds = vector<int>{ 0, 3, 5 };

				else if (userSection_numEdge == 9)temp_CornerIds = vector<int>{ 0, 3, 6 };
			...
			for (int i = 0; i < eFaces.size(); i++)
			{
				...
				if (gotFace != volumeFace_PrimalFace.end())
				{
					...
					if (!GFP_SSP_Face[globalFaceId])
					{
						...
						zVector form_eCen = form_e.getCenter();
						zVector fCenter = fCenters[eFaces[i].getId()];
						...
						for (int j = 0; j < tempPosns.size(); j++)
						{
							...
							zVector newPos = sectionPoints[closestId];
	...
	ZSPACE_INLINE void zTsPolytopal::getPolytopal_Beams_Profile(int user_nEdges, double user_edgeLength, double param)
	{
		...
		for (int i = 0; i < primal_n_f_i; i++)
		{
			...
			{
				zVector newPos0 = p - (eVec * 0.5 * eLength * param);
				zVector newPos1 = p + (eVec * 0.5 * eLength * param);
	...
	ZSPACE_INLINE void zTsPolytopal::setVertexOffsets(vector<double> &offsets)
	{
	...
	ZSPACE_INLINE void zTsPolytopal::get_BMatrix(mat &A, double threshold, int &rank, mat &B, vector<bool> &indEdges)
	{
	...
	ZSPACE_INLINE void zTsPolytopal::getPolytopalRulingRemesh(int index, zObjMesh &inMeshObj, int SUBDIVS)
	{
		...
		for (int i = 0; i < n_v_lowPoly; i += 6)
		{
			...
			zVector v0 = vert0.getPosition();

			zVector v1 = vert1.getPosition();
			...
			for (int k = 0; k < pow(2, (SUBDIVS + 1)); k++)
			{
				...
				zVector v2 = edge0.getStartVertex().getPosition(); ;

				zVector v3 = edge1.getVertex().getPosition();
	...
	ZSPACE_INLINE bool zTsPolytopal::computeRulingIntersection(int polytopalIndex, zItMeshVertex &v0, zItMeshVertex &v1, zVector &closestPt)
	{
		bool out = false;
		...
		if (e0HasEdge  && e1HasEdge)
		{
			...
			zVector a0 = v2.getPosition();

			zVector a1 = v0.getPosition();

			zVector b0 = v3.getPosition();

			zVector b1 = v1.getPosition();
	...
	ZSPACE_INLINE void zTsPolytopal::get_YVertex(vector<int> &Y_vertex)
	{
		Y_vertex.clear();
	...
	ZSPACE_INLINE void zTsPolytopal::getUserSection(int face_numEdges, vector<zVector> &sectionPoints)
	{
		sectionPoints.clear();
		...
		if (face_numEdges == 3)
		{
			...
			{
				sectionPoints.push_back(zVector(0, halfElen, 0));
				sectionPoints.push_back(zVector(-halfElen, halfElen, 0));
				sectionPoints.push_back(zVector(-halfElen, -halfElen, 0));
				sectionPoints.push_back(zVector(0, -halfElen, 0));
				sectionPoints.push_back(zVector(halfElen, -halfElen, 0));
				sectionPoints.push_back(zVector(halfElen, halfElen, 0));
			...
			if (userSection_numEdge == 5)
			{
				...
				for (int i = 0; i < numPoints; i++)
				{
					...
					if (i == 3)
					{
						zVector pos1 = (sectionPoints[i - 1] + pos) * 0.5;
