...
namespace zSpace
{
	...
	ZSPACE_INLINE void zGraph::create(vector<zVector>(&_positions), vector<int>(&edgeConnects), bool staticGraph )
	{
		// clear containers
		clear();
	...
	ZSPACE_INLINE void zGraph::create(vector<zVector>(&_positions), vector<int>(&edgeConnects), zVector &graphNormal, zVector &sortReference)
	{
	...
	ZSPACE_INLINE bool zGraph::addVertex(zVector &pos)
	{
		bool out = false;
	...
	ZSPACE_INLINE bool zGraph::vertexExists(zVector pos, int &outVertexId, int precisionfactor)
	{
		bool out = false;;
		outVertexId = -1;
	...
	ZSPACE_INLINE void zGraph::addToPositionMap(zVector &pos, int index, int precisionfactor)
	{
		double factor = pow(10, precisionfactor);
		double x = std::round(pos.x *factor) / factor;
		double y = std::round(pos.y *factor) / factor;
	...
	ZSPACE_INLINE void zGraph::removeFromPositionMap(zVector &pos, int precisionfactor)
	{
		double factor = pow(10, precisionfactor);
		double x = std::round(pos.x *factor) / factor;
		double y = std::round(pos.y *factor) / factor;
	...
	ZSPACE_INLINE void zGraph::cyclic_sortEdges(vector<int> &unSortedEdges, zVector &center, int sortReferenceIndex, vector<int> &sortedEdges)
	{
	...
	ZSPACE_INLINE void zGraph::cyclic_sortEdges(vector<int> &unSortedEdges, zVector &center, zVector& referenceDir, zVector& norm, vector<int> &sortedEdges)
	{
