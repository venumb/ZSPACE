...
namespace zSpace
{
	...
	ZSPACE_INLINE void zPointCloud::create(vector<zVector> &_positions)
	{
		// clear containers
		clear();
	...
	ZSPACE_INLINE bool zPointCloud::addVertex(zVector &pos)
	{
		bool out = false;
	...
	ZSPACE_INLINE bool zPointCloud::vertexExists(zVector pos, int &outVertexId, int precisionfactor)
	{
		bool out = false;;
		outVertexId = -1;
	...
	ZSPACE_INLINE void zPointCloud::addToPositionMap(zVector &pos, int index, int precisionfactor)
	{
		double factor = pow(10, precisionfactor);
		double x = std::round(pos.x *factor) / factor;
		double y = std::round(pos.y *factor) / factor;
	...
	ZSPACE_INLINE void zPointCloud::removeFromPositionMap(zVector &pos, int precisionfactor)
	{
		double factor = pow(10, precisionfactor);
		double x = std::round(pos.x *factor) / factor;
		double y = std::round(pos.y *factor) / factor;
