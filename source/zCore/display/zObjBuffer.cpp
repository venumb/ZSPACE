...
namespace zSpace
{
	...
	ZSPACE_INLINE int zObjBuffer::appendVertexAttributes(zVector*_positions, zVector* _normals, int size)
	{
	...
	ZSPACE_INLINE int zObjBuffer::appendEdgeIndices(vector<int> &_edgeIndicies)
	{
		int out = nEdges;
	...
	ZSPACE_INLINE int zObjBuffer::appendFaceIndices(vector<int> &_faceIndicies)
	{
		int out = nFaces;
	...
	ZSPACE_INLINE void zObjBuffer::updateVertexPositions(zVector* _positions, int size, int startId)
	{
